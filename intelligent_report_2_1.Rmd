...
subtitle: "Intelligent Statistical Analysis: Metabolomic analysis for 2 groups using the selected 'Pre-processing' by user"
...
### Metabolites with NORMAL distribution & variance HOMOSCEDASTICITY 

```{r, echo = FALSE, warning = FALSE, comment = NA, message=FALSE}
## NORMAL & HOMO

Group <- as.factor(data_no_ho$Group)
...
if(ncol(data_no_ho) == 3){
  ...
  p %>%
    ...
    dplyr::select(Metabolite, `Mean G1`, `Mean G2`, `FC (Ratio)`, `Difference of Means`, P.Value) %>%
    kable(format = "html", escape = F) %>%
    kable_styling("striped", full_width = F)
}
```

### Metabolites with NORMAL distribution & variance HETEROSCEDASTICITY

```{r, echo = FALSE, warning = FALSE, comment = NA, message=FALSE}
## NORMAL & HETERO

Group <- as.factor(data_no_he$Group)
...
### Metabolites with NORMAL distribution ANOVA model

```{r, echo = FALSE, warning = FALSE, comment = NA, message=FALSE}
data_anova <- cbind(data_no_ho, data_no_he[,-c(1,2)])

if(ncol(data_no_ho) == 3){
  colnames(data_anova)[3] <- colnames(data_no_ho)[3]
}

if(ncol(data_no_he) == 3){
  colnames(data_anova)[ncol(data_anova)] <- colnames(data_no_he)[3]
}

Group <- as.factor(data_anova$Group)
